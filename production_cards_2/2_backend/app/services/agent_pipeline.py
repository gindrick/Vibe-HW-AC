...
async def _execute_tool(
    ...
    """
    Execute one supervisor tool call.
    pipeline_state holds document_reading and draft so the supervisor
    doesn't need to pass large JSON objects as tool arguments.
    ...
    try:
        if tool_name == "read_document":
            result = await asyncio.wait_for(
                _run_document_reader(client, tool_input.get("png_path", png_path), pdf_name),
                timeout=SUBAGENT_TIMEOUT_SECONDS,
            )
            pipeline_state["document_reading"] = result
            content = json.dumps(result, ensure_ascii=False)

        elif tool_name == "extract_form":
            document_reading = tool_input.get("document_reading") or pipeline_state.get("document_reading")
            if not document_reading:
                content = json.dumps({"error": "No document_reading available. Call read_document first."})
            else:
                patches = tool_input.get("patches")
                result = await asyncio.wait_for(
                    _run_form_extractor(client, document_reading, pdf_name, patches=patches),
                    timeout=SUBAGENT_TIMEOUT_SECONDS,
                )
                pipeline_state["draft"] = result
                content = json.dumps(result, ensure_ascii=False)

        elif tool_name == "review_draft":
            document_reading = tool_input.get("document_reading") or pipeline_state.get("document_reading")
            draft = tool_input.get("draft") or pipeline_state.get("draft")
            if not document_reading or not draft:
                missing = []
                if not document_reading:
                    missing.append("document_reading")
                if not draft:
                    missing.append("draft")
                content = json.dumps({"error": f"Missing state: {missing}. Call read_document and extract_form first."})
            else:
                result = await asyncio.wait_for(
                    _run_validator_reviewer(
                        client,
                        tool_input.get("png_path", png_path),
                        document_reading,
                        draft,
                        pdf_name,
                    ),
            ...
            draft = tool_input.get("draft") or pipeline_state.get("draft")
            if not draft:
                content = json.dumps({"error": "No draft available. Call extract_form first."})
            else:
                status = tool_input.get("status", "success")
                model_label = f"{settings.agent_model}/supervisor-llm/{status}"
                finalized = _finalize(draft, model_label)
...
async def extract_card_data(pdf_path: str) -> dict:
    ...
    pipeline_state: dict = {}  # shared state: document_reading, draft
    ...
    try:
        ...
        for iteration in range(SUPERVISOR_MAX_ITERATIONS):
            ...
            # Execute tool calls — pipeline_state is shared and mutated serially
            # (parallel execution safe: only read_document/extract_form write to state,
            #  and the supervisor rarely issues them in parallel)
            raw_results = await asyncio.gather(
                *[_execute_tool(tc, client, str(png_path), pdf_name, iteration, pipeline_state)
